...
out=ROOT/'_build/hardening-normal';out.mkdir(exist_ok=False)
source=out/'source'
...
common=['-G','Ninja','-DCMAKE_BUILD_TYPE=Release','-DCMAKE_OSX_DEPLOYMENT_TARGET=13.3','-DCMAKE_EXPORT_COMPILE_COMMANDS=ON']
install=ROOT/'_build/native/install'
run(['cmake','-S',str(source),'-B',str(out/'ladybug'),*common,'-DBUILD_SHELL=OFF','-DBUILD_TESTS=OFF','-DBUILD_SINGLE_FILE_HEADER=OFF','-DEXTENSION_STATIC_LINK_LIST=fts;vector;duckdb','-DBUILD_SHARED_LBUG=ON','-DBUILD_STATIC_LBUG=ON',f'-DDuckDB_DIR={ROOT / "_build/native/duckdb"}',f'-DOPENSSL_ROOT_DIR={install}','-DOPENSSL_USE_STATIC_LIBS=ON',f'-DOPENSSL_SSL_LIBRARY={install / "lib/libssl.a"}',f'-DOPENSSL_CRYPTO_LIBRARY={install / "lib/libcrypto.a"}',f'-DOPENSSL_INCLUDE_DIR={install / "include"}'])
